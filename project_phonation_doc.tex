...
Clearly in order for a stationary point \(b_0\) to have real eigenvalues, we must have \(f'(b_0) \ge 0\).
On equality, eigenvalues will be zero. This condition can be written more explicitly as \(\mu q^2 \ge b_0^3\), which is a surprisingly simple statement.

Finding the case in which there is exactly one equilibrium solution is difficult,
since the equation which determines equilibria is non-linear.
Analytical solutions cannot be found using standard methods,
and numerical solutions cannot be accurate enough to provide analytically sound results.



%eigenvalues at the single equilibrium

%plot of eigenvalues as they converge
...
This means the curve \(F(b)\) is also zero at the stationary point \(b = b-1\).
...
The value $b_1$ is, trivially, the minimal positive zero of $f(b)$ which we have already computed to be $b_1 \approx 0.1795$.
...
Recall Equation \ref{eqn:osc_condition}, in this case we can find the kinetic energy curve takes this value on the optimal intersection and we get the result we want! %this sentence is not meant to stay
\dots
