...
We want \(f((\mu q^2)^(1/3))\) > 0. Expand and rearrange to obtain an explicit expression, being
...
\begin{align}
    \frac{db}{dt}       & = p                          \\
    \frac{dp}{dt} & = 1 - q - b - \frac{\mu q^2}{2b^2}.
...
\begin{figure}
    \centering
    \includegraphics[width = 0.5\linewidth]{figures/stable_oscillations.eps}
    \caption{
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/trajectories_unstable.eps}
    \caption{
...
Figure \ref{fig:equilibrium_eigenvalues} shows the equilibrium solutions as the parameter $q$ changes,
and plots their eigenvalues in the complex plane as the equilibrium solutions converge.

In the proximity of $b_2$, we observe oscillations about the fixed point. In proximity to $b_1,$ however,
a variety of behaviours can be observed. Figures \ref{fig:stable_oscillations} and \ref{fig:unstable_trajectories} show the behaviours local to each equilibrium solution.
...
This condition can be expanded and written more explicitly as \(\mu q^2 \ge b_0^3\), which is a surprisingly simple statement.
