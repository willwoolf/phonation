...
\begin{equation}
    \begin{aligned}
        \frac{1}{2}\rho U_0^2 + p_0 &= \rho\mathrm{E} &\text{(lower airway)}\\
        \frac{1}{2}\rho U_1^2 + p_1 &= \rho\mathrm{E} &\text{(volume $V_1$)}  \\
        \frac{1}{2}\rho U_2^2 + p_2 &= \rho\mathrm{E} &\text{(volume $V_2$)}  \\
        \frac{1}{2}\rho U_\infty^2 + p_\infty &= \rho\mathrm{E} &\text{(upper airway)},
    \end{aligned}
\end{equation}

where $\mathrm{E}$ is the Bernoulli energy constant on the streamline.
We can rearrange and obtain explicit expressions for pressure, being

\begin{equation}
    \begin{aligned}
        p_0 = \rho\left(\mathrm{E} - \frac{1}{2}U_0^2\right) \\
        p_1 = \rho\left(\mathrm{E} - \frac{1}{2}U_1^2\right) \\
        p_2 = \rho\left(\mathrm{E} - \frac{1}{2}U_2^2\right) \\
        p_\infty = \rho\left(\mathrm{E} - \frac{1}{2}U_\infty^2\right).
    \end{aligned}
\end{equation}

In the upper airway we have $p_\infty$ local to the vocal opening,
hence this is \textit{atmospheric pressure}, which we will write as $\mathrm{p}$
In the regions of interest, we are concerned with the difference in pressure from atmospheric level,
hence we express the differences:

\begin{equation}
    \begin{aligned}
        \delta p_0 &= \rho\mathrm{E} - \rho\frac{1}{2}U_0^2 - \rho\mathrm{E} + \frac{1}{2}U_\infty^2 = \frac{1}{2}\rho\left(U_\infty^2 - U_0^2\right) \\
        \delta p_1 &= \frac{1}{2}\rho\left(U_\infty^2 - U_1^2\right) \\
        \delta p_2 &= \frac{1}{2}\rho\left(U_\infty^2 - U_2^2\right).
    \end{aligned}
\end{equation}
