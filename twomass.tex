...
\subsection{Fluid flow and pressure}
...
\subsection{Solid mechanics of springs and stiffness}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where $b*$ is the resting position of both masses. It will be easier to make $h$ the variable of interest, since closure at $h=0$ is identifiable,
...
\begin{figure}
    ...
    \caption{
        ...
        Parameters \((1,1,3,20)\) iNumerical methods do not provide exact results,
        and it is very easy to produce results which seem plausible but are fundamentally wrong.nduces a strong coupling which replicates the behaviour of the single mass model.
        Perturbations of distance of between $h_1$ and $h_2$ of order $10^{-2}$ induce steady microoscillations
    }
...
When producing Figures \ref{fig:twomass_energy_madplot}, \ref{fig:twomass_energy_2} and \ref{fig:twomass_energy_3}, we have reduced the tolerances to meet this criterion,
such that we can claim these results are reliable.
...
\begin{figure}
	...
	\caption{
		...
		Computation initialised with parameters \((1, 0.5, 3, 0.3)\) and initial positions \((1,3.79)\).
		We observe continuous orbits.
	}
...
\begin{figure}
	...
	\label{fig:twomass_fourier_goodcase}
\end{figure}
...
Figure \ref{fig:twomass_fourier_goodcase} applies the transform to data we are familiar with, having extended the time scale.
requency space is computed as the $n$ points in the time array divided by the total time span.
Since the time series data is produced by a non-uniform timestep,
we use the \texttt{nufft()} function in MATLAB,
which is a non-uniform fast Fourier transform algorithm that requires the time series array and the time array.
...
%comments on the fourier transforms
